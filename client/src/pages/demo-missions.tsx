...
export default function DemoMissions() {
  ...
  const start = async (mission: DemoMissionView) => { await demoApi.startMission(mission.definition.missionId); const offer = await demoApi.getOffer(mission.definition.offerId).catch(() => null); if (offer) navigate(`/demo/organizations/${encodeURIComponent(offer.organizationId)}?mission=${encodeURIComponent(mission.definition.missionId)}`); };
