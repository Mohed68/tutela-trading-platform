...
export default function Register() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (form.password.length < 10) return setError("Password must contain at least 10 characters.");
    setLoading(true);
    try {
      const response = await apiRequest("POST", "/api/auth/register", { firstName: form.firstName, lastName: form.lastName, email: form.email, password: form.password });
      queryClient.setQueryData(["/api/auth/user"], await response.json()); navigate("/");
