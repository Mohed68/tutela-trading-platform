    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8" data-testid="secure-admin-control-plane">
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        ...
        <main className="space-y-5">
          ...
          {section==="trade-operations"&&<div className="space-y-5"><p className="rounded border bg-white p-3 text-sm">ACTIVE_BASELINE — safe Current V2 plus bounded MVP Contract/signature/execution visibility. Admin can inspect state and integrity metadata but cannot fabricate party signatures, delivery, settlement, or Contract terms.</p>{(["offers","orders","contracts"] as const).map(kind=><Card key={kind}><CardHeader><CardTitle className="capitalize">{kind}</CardTitle></CardHeader><CardContent><SafeRows rows={tradeOperations[kind].map(row=>Object.entries(row).map(([key,value])=>`${key}: ${String(value??"—")}`).join(" · "))}/></CardContent></Card>)}</div>}
