...
export default function DemoOrganizationPage() {
  ...
  const continueFlow = async () => { if (!mission) return navigate("/demo/marketplace"); const refreshed=await demoApi.getMission(mission.definition.missionId); setMission(refreshed); navigate(`/demo/offers/${encodeURIComponent(refreshed.definition.offerId)}?mission=${encodeURIComponent(refreshed.definition.missionId)}`); };
