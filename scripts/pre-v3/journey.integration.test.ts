...
test("real MVP journey: verified Urea offer through executed contract and trade closeout", {timeout:900_000}, async(t)=>{
  ...
  try{
    ...
    async function verifyOrganization(owner:string,label:string){
      ...
      const registrationIdentifier=`${label.replaceAll(" ","-").toLowerCase()}-${suffix}`;
      const created=await app.createOrganization(owner,{legalName:label,tradingNames:[],organizationType:"company",jurisdiction:"AE",registrationIdentifiers:[{scheme:"license",value:registrationIdentifier}],declaredActivities:[{code:commodity.type}]});assert.equal(created.status,"created");
      const organizationId=created.organizationId!,profileRevisionId=created.profileRevisionId!;
      const assertions=[{assertionCode:"evidence_category",value:"organization_existence"},{assertionCode:"evidence_category",value:"representative_association"},{assertionCode:"document_type",value:"business_registration"},{assertionCode:"registration_identifier",value:`license:${registrationIdentifier}`},{assertionCode:"legal_name",value:label},{assertionCode:"registration_jurisdiction",value:"AE"},{assertionCode:"association_asserted",value:"true"},{assertionCode:"representative_reference",value:owner},{assertionCode:"organization.activity_code",value:commodity.type},{assertionCode:"activity.commodity_id",value:commodity.id},{assertionCode:"activity.commodity_classification",value:commodity.type}];
