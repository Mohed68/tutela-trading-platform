...
test('TEST PostgreSQL: independent review, canonical V2 replay, risk separation, explicit reversible enforcement and atomic audit', {timeout:300_000},async()=>{
  ...
  try {
    ...
    const decision={caseId:firstCase.id,expectedActionId:null,state:'RESTRICTED',reason:'Governed scope review',remediation:'Provide independently confirmed revised evidence',reviewAt:new Date(Date.now()+86_400_000).toISOString()};
    ...
    assert.equal(action.integrationStatus,'INACTIVE');
    ...
    await assert.rejects(service.decide(context(),{...decision,caseId:liftingCase.id,state:'NORMAL'}));
    await service.decide(context(),{...decision,caseId:liftingCase.id,expectedActionId:action.id,state:'NORMAL',reason:'Confirmed remediation; explicitly lift prior restriction'});
