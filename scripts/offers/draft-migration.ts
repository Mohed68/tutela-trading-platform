...
const MIGRATION_IDENTIFIER = "0007_add_draft_offer_status";
const MIGRATION_FILENAME = "migrations/0007_add_draft_offer_status.sql";
...
async function enumValues(client: Client): Promise<string[]> {
  return (
...
async function verifyProtectedState(
  ...
  if (fingerprint !== expectedFingerprint) {
    throw new Error("OFFER_DRAFT_FINGERPRINT_MISMATCH");
  }
  ...
  const statuses = await enumValues(client);
...
async function verifyJournal(client: Client): Promise<void> {
  ...
  if (
    ...
    throw new Error("OFFER_DRAFT_MIGRATION_JOURNAL_INVALID");
  }
}
...
async function rehearse(client: Client): Promise<void> {
  ...
  try {
    ...
    if (!statuses.includes("draft")) {
      throw new Error("DRAFT_ENUM_VALUE_NOT_ADDED");
    }
...
async function execute(client: Client): Promise<void> {
  ...
  try {
    ...
    if (existingJournal !== "0") {
      throw new Error("OFFER_DRAFT_MIGRATION_JOURNAL_COLLISION");
    }
    ...
    await client.query(
      ...
      [
        ...
        gitRevision(),
        "Phase 5B additive draft enum value; no default change or backfill.",
      ],
    );
  ...
  try {
    ...
    const draftCast = (
      await client.query<{ value: string }>(
        "SELECT 'draft'::public.offer_status::text AS value",
      )
    ).rows[0].value;
    if (draftCast !== "draft") {
      throw new Error("DRAFT_ENUM_CAST_FAILED");
    }
    await client.query("ROLLBACK");
    console.log(
      JSON.stringify({
...
if (!command || !["rehearse", "execute", "verify"].includes(command)) {
  console.error("OFFER_DRAFT_MIGRATION_COMMAND_REQUIRED");
  process.exit(1);
}
