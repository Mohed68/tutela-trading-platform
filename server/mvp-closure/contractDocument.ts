...
const GENERAL_CONDITIONS:readonly [string,string][]=Object.freeze([
  ...
  ["Contract formation","This Contract is formed only when both authorized parties sign the same version through TUTELA. A preview or generated PDF alone is not acceptance."],
...
function buildLines(snapshot:ContractTermsSnapshot,signatures:readonly ContractDocumentSignature[],executed:boolean):Line[]{
  const l:Line[]=[];add(l,"TUTELA International Commodity Sale Contract - v1",17,true,7);
  add(l,executed?"EXECUTED CONTRACT ARTIFACT":"CONTRACT PREVIEW - NOT YET EXECUTED",11,true,7);
  field(l,"Contract number",snapshot.tutelaReference);field(l,"Contract ID",snapshot.contractId);field(l,"Contract version",snapshot.contractVersion);
  field(l,"Contract Snapshot ID",snapshot.snapshotId);field(l,"Template version",snapshot.templateVersion);field(l,"Commercial profile version",snapshot.commercialProfileVersion);
  field(l,"Snapshot integrity",sha256({scope:"mvp-contract-snapshot/v1",snapshot}));
  add(l,"This document is generated from the server-authoritative snapshot. PDF generation does not itself form, amend or execute the Contract.",8,false,6);
  section(l,"PART A - SPECIFIC CONDITIONS");
  field(l,"Contract date",snapshot.snapshotCreatedAt);field(l,"Seller",`${snapshot.seller.legalName} (${snapshot.seller.registrationJurisdiction})`);
  field(l,"Seller identifiers",snapshot.seller.registrationIdentifiers.map(v=>`${v.scheme}: ${v.value}`).join("; "));field(l,"Seller representative",snapshot.seller.authorizedRepresentative);
  field(l,"Buyer",`${snapshot.buyer.legalName} (${snapshot.buyer.registrationJurisdiction})`);field(l,"Buyer identifiers",snapshot.buyer.registrationIdentifiers.map(v=>`${v.scheme}: ${v.value}`).join("; "));field(l,"Buyer representative",snapshot.buyer.authorizedRepresentative);
  field(l,"Commodity",`${snapshot.commodity.name} - ${snapshot.commodity.grade}`);field(l,"Product description",snapshot.commodity.productDescription);field(l,"Origin",snapshot.commodity.origin);if(snapshot.commodity.producer)field(l,"Producer",snapshot.commodity.producer);
  field(l,"Quantity",`${snapshot.quantity.amount} ${snapshot.quantity.unit} +/- ${snapshot.quantity.tolerancePercent}%`);field(l,"Price",`${snapshot.price.currency} ${snapshot.price.unitPrice} per ${snapshot.quantity.unit}`);field(l,"Pricing basis",snapshot.price.pricingBasis);field(l,"Estimated contract value",`${snapshot.price.currency} ${snapshot.price.estimatedTotal}`);
  field(l,"Delivery",`${snapshot.delivery.incoterm} ${snapshot.delivery.namedPlace} Incoterms (R) ${snapshot.delivery.incotermsVersion}`);field(l,"Shipment window",`${snapshot.delivery.shipmentWindowStart} to ${snapshot.delivery.shipmentWindowEnd}`);field(l,"Packaging",snapshot.delivery.packaging);field(l,"Partial shipments",snapshot.delivery.partialShipmentPolicy);
  field(l,"Inspection",snapshot.inspection.required?`${snapshot.inspection.bodyOrMethod}; ${snapshot.inspection.inspectionPoint}; ${snapshot.inspection.finalityAndClaims}`:"Not required by agreement");field(l,"Payment",`${snapshot.payment.method}; ${snapshot.payment.timing}; ${snapshot.payment.currency}; ${snapshot.payment.bankDocumentConditions}`);
  ...
  field(l,"Force majeure treatment",snapshot.legal.forceMajeureTreatment);field(l,"Hardship treatment",snapshot.legal.hardshipTreatment);field(l,"Platform fee treatment",snapshot.platform.platformFeeTreatment);field(l,"Special conditions",snapshot.specialConditions.join("; ")||"None");
  section(l,"PART B - GENERAL CONDITIONS");for(const [i,[title,body]] of GENERAL_CONDITIONS.entries())add(l,`${i+1}. ${title}. ${body}`,8.5,false,4);
  ...
  section(l,"ANNEX B - DELIVERY / SHIPMENT SCHEDULE");field(l,"Shipment window",`${snapshot.delivery.shipmentWindowStart} to ${snapshot.delivery.shipmentWindowEnd}`);field(l,"Named place",snapshot.delivery.namedPlace);field(l,"Packaging",snapshot.delivery.packaging);
  ...
  section(l,"SIGNATURES");for(const party of ["SELLER","BUYER"] as const){const s=signatures.find(v=>v.party===party);field(l,party,s?`${s.signerName}, for ${s.organizationName}, signed ${s.signedAt}, integrity ${s.signatureFingerprint}`:"Awaiting authorized electronic signature")}
...
function makePdf(lines:Line[]):{bytes:Buffer;pageCount:number}{
  const pages:Line[][]=[];let page:Line[]=[];let y=770;for(const line of lines){const needed=line.size+line.gap;if(y-needed<55){pages.push(page);page=[];y=770}page.push(line);y-=needed}if(page.length)pages.push(page);
  ...
  const pageIds:number[]=[];for(const [pageIndex,pageLines] of pages.entries()){
    let cursor=790;const ops:string[]=["0.12 0.18 0.16 rg","0.5 w","50 805 m 545 805 l S"];
    ...
    ops.push("BT","/F1 8 Tf",`1 0 0 1 55 28 Tm`,`(TUTELA International Commodity Sale Contract - v1) Tj`,"ET","BT","/F1 8 Tf",`1 0 0 1 490 28 Tm`,`(Page ${pageIndex+1} of ${pages.length}) Tj`,"ET");
...
export function generateContractPdf(snapshot:ContractTermsSnapshot,signatures:readonly ContractDocumentSignature[]=[],executed=false):ContractDocument{
  const result=makePdf(buildLines(snapshot,signatures,executed));return Object.freeze({...result,sha256:sha256(result.bytes)});
}
