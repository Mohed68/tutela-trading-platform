...
const preparation=z.object({
  sellerRepresentative:bounded(200),buyerRepresentative:bounded(200),sellerAddress:address,buyerAddress:address,grade:bounded(100),productDescription:bounded(2000),origin:bounded(200),producer:bounded(200).optional(),specifications:z.array(specification).min(1).max(40),
  quantityTolerancePercent:z.string().regex(/^\d{1,2}(?:\.\d{1,2})?$/),pricingBasis:bounded(500),incoterm:z.enum(INCOTERMS_2020),namedPlace:bounded(300),shipmentWindowStart:z.string().datetime(),shipmentWindowEnd:z.string().datetime(),packaging:bounded(500),partialShipmentPolicy:z.enum(["ALLOWED","NOT_ALLOWED","BY_WRITTEN_AGREEMENT"]),
  inspection:z.object({required:z.boolean(),bodyOrMethod:z.string().trim().max(500),inspectionPoint:z.string().trim().max(500),quantityDetermination:z.string().trim().max(500),qualityDetermination:z.string().trim().max(500),finalityAndClaims:z.string().trim().max(1000)}).strict(),
  payment:z.object({method:bounded(300),timing:bounded(500),currency:z.string().trim().regex(/^[A-Z]{3}$/),bankDocumentConditions:z.string().trim().max(1000)}).strict(),requiredDocuments:z.array(bounded(200)).min(1).max(30),
