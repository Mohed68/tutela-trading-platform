...
export function registerMvpClosureRoutes(app:Express,pool:Pool):ReturnType<typeof createMvpClosureService>{
  ...
  app.post("/api/organizations/:organizationId/contract-signing-authorities",auth,(_req,res)=>res.status(410).json({message:"Binary signing-authority grants are retired. Use a scoped Organization Signing Mandate.",code:"legacy_signing_authority_grant_retired"}));
