...
test("Control Plane overview is bounded and labels operational VRE modules honestly", async () => {
  ...
  assert.equal(result.modules.find((item) => item.id === "verification")?.maturity, "ACTIVE_BASELINE");
  assert.equal(result.modules.find((item) => item.id === "risk")?.maturity, "ACTIVE_BASELINE");
  assert.equal(result.modules.find((item) => item.id === "enforcement")?.maturity, "ACTIVE_BASELINE");
...
test("Control Plane identity rows never select raw user or credential fields", async () => {
  ...
  assert.doesNotMatch(sql, /SELECT\s+\*|password|phone|address|session|encrypted_secret|recovery_code/i);
